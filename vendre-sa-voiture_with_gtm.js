let Matches = 0;
const vehicules = [{
"Marque": "abarth",
"Critère Modèle": "matchAll",
"Modèle": "",
"Critère année": "",
"Année": null,
"Critère Kilométrage": "lessThan",
"Kilométrage": 175000
},
{
"Marque": "ac",
"Critère Modèle": "contains",
"Modèle": "cobra",
"Critère année": "",
"Année": null,
"Critère Kilométrage": "lessThan",
"Kilométrage": 175000
},
{
"Marque": "ac",
"Critère Modèle": "contains",
"Modèle": "bristol",
"Critère année": "",
"Année": null,
"Critère Kilométrage": "lessThan",
"Kilométrage": 175000
},
{
"Marque": "alfa romeo",
"Critère Modèle": "contains",
"Modèle": "gta",
"Critère année": "",
"Année": null,
"Critère Kilométrage": "lessThan",
"Kilométrage": 175000
},
{
"Marque": "alfa romeo",
"Critère Modèle": "contains",
"Modèle": "gtv",
"Critère année": "",
"Année": null,
"Critère Kilométrage": "lessThan",
"Kilométrage": 175000
},
{
"Marque": "alfa romeo",
"Critère Modèle": "isExactly",
"Modèle": "sz",
"Critère année": "",
"Année": null,
"Critère Kilométrage": "lessThan",
"Kilométrage": 175000
},
{
"Marque": "alfa romeo",
"Critère Modèle": "isExactly",
"Modèle": "rz",
"Critère année": "",
"Année": null,
"Critère Kilométrage": "lessThan",
"Kilométrage": 175000
},
{
"Marque": "alfa romeo",
"Critère Modèle": "isExactly",
"Modèle": "tz",
"Critère année": "",
"Année": null,
"Critère Kilométrage": "lessThan",
"Kilométrage": 175000
},
{
"Marque": "alfa romeo",
"Critère Modèle": "contains",
"Modèle": "montreal",
"Critère année": "",
"Année": null,
"Critère Kilométrage": "lessThan",
"Kilométrage": 175000
},
{
"Marque": "alfa romeo",
"Critère Modèle": "contains",
"Modèle": "1300",
"Critère année": "",
"Année": null,
"Critère Kilométrage": "lessThan",
"Kilométrage": 175000
},
{
"Marque": "alfa romeo",
"Critère Modèle": "contains",
"Modèle": "1600",
"Critère année": "",
"Année": null,
"Critère Kilométrage": "lessThan",
"Kilométrage": 175000
},
{
"Marque": "alfa romeo",
"Critère Modèle": "contains",
"Modèle": "1750",
"Critère année": "",
"Année": null,
"Critère Kilométrage": "lessThan",
"Kilométrage": 175000
},
{
"Marque": "alfa romeo",
"Critère Modèle": "contains",
"Modèle": "2000",
"Critère année": "",
"Année": null,
"Critère Kilométrage": "lessThan",
"Kilométrage": 175000
},
{
"Marque": "alfa romeo",
"Critère Modèle": "contains",
"Modèle": "bertone",
"Critère année": "",
"Année": null,
"Critère Kilométrage": "lessThan",
"Kilométrage": 175000
},
{
"Marque": "ariel",
"Critère Modèle": "matchAll",
"Modèle": "",
"Critère année": "",
"Année": null,
"Critère Kilométrage": "lessThan",
"Kilométrage": 175000
},
{
"Marque": "aston martin",
"Critère Modèle": "matchAll",
"Modèle": "",
"Critère année": "",
"Année": null,
"Critère Kilométrage": "lessThan",
"Kilométrage": 175000
},
{
"Marque": "alpine",
"Critère Modèle": "matchAll",
"Modèle": "",
"Critère année": "",
"Année": null,
"Critère Kilométrage": "lessThan",
"Kilométrage": 175000
},
{
"Marque": "audi",
"Critère Modèle": "startsWith",
"Modèle": "rs",
"Critère année": "",
"Année": null,
"Critère Kilométrage": "lessThan",
"Kilométrage": 120000
},
{
"Marque": "audi",
"Critère Modèle": "isExactly",
"Modèle": "quattro",
"Critère année": "",
"Année": null,
"Critère Kilométrage": "lessThan",
"Kilométrage": 175000
},
{
"Marque": "audi",
"Critère Modèle": "contains",
"Modèle": "r8",
"Critère année": "",
"Année": null,
"Critère Kilométrage": "lessThan",
"Kilométrage": 130000
},
{
"Marque": "austin healey",
"Critère Modèle": "matchAll",
"Modèle": "matchAll",
"Critère année": "",
"Année": null,
"Critère Kilométrage": "lessThan",
"Kilométrage": 175000
},
{
"Marque": "bentley",
"Critère Modèle": "matchAll",
"Modèle": "matchAll",
"Critère année": "",
"Année": null,
"Critère Kilométrage": "lessThan",
"Kilométrage": 175000
},
{
"Marque": "bmw",
"Critère Modèle": "startsWith",
"Modèle": "m",
"Critère année": "",
"Année": null,
"Critère Kilométrage": "lessThan",
"Kilométrage": 175000
},
{
"Marque": "bmw",
"Critère Modèle": "contains",
"Modèle": "z1",
"Critère année": "",
"Année": null,
"Critère Kilométrage": "lessThan",
"Kilométrage": 100000
},
{
"Marque": "bmw",
"Critère Modèle": "contains",
"Modèle": "z8",
"Critère année": "",
"Année": null,
"Critère Kilométrage": "lessThan",
"Kilométrage": 100000
},
{
"Marque": "bmw",
"Critère Modèle": "contains",
"Modèle": "z3",
"Critère année": "",
"Année": null,
"Critère Kilométrage": "lessThan",
"Kilométrage": 175000
},
{
"Marque": "bmw",
"Critère Modèle": "contains",
"Modèle": "z4",
"Critère année": "",
"Année": null,
"Critère Kilométrage": "lessThan",
"Kilométrage": 150000
},
{
"Marque": "bmw",
"Critère Modèle": "matchAll",
"Modèle": "",
"Critère année": "lessThan",
"Année": 1990,
"Critère Kilométrage": "lessThan",
"Kilométrage": 175000
},
{
"Marque": "alpina",
"Critère Modèle": "matchAll",
"Modèle": "",
"Critère année": "",
"Année": null,
"Critère Kilométrage": "lessThan",
"Kilométrage": 175000
},
{
"Marque": "bugatti",
"Critère Modèle": "matchAll",
"Modèle": "",
"Critère année": "",
"Année": null,
"Critère Kilométrage": "lessThan",
"Kilométrage": 100000
},
{
"Marque": "caterham",
"Critère Modèle": "matchAll",
"Modèle": "",
"Critère année": "",
"Année": null,
"Critère Kilométrage": "lessThan",
"Kilométrage": 175000
},
{
"Marque": "chevrolet",
"Critère Modèle": "contains",
"Modèle": "camaro",
"Critère année": "",
"Année": null,
"Critère Kilométrage": "lessThan",
"Kilométrage": 175000
},
{
"Marque": "chevrolet",
"Critère Modèle": "contains",
"Modèle": "corvette",
"Critère année": "",
"Année": null,
"Critère Kilométrage": "lessThan",
"Kilométrage": 175000
},
{
"Marque": "datsun",
"Critère Modèle": "contains",
"Modèle": "",
"Critère année": "",
"Année": null,
"Critère Kilométrage": "lessThan",
"Kilométrage": 175000
},
{
"Marque": "dodge",
"Critère Modèle": "contains",
"Modèle": "viper",
"Critère année": "",
"Année": null,
"Critère Kilométrage": "lessThan",
"Kilométrage": 175000
},
{
"Marque": "dodge",
"Critère Modèle": "contains",
"Modèle": "challenger",
"Critère année": "",
"Année": null,
"Critère Kilométrage": "lessThan",
"Kilométrage": 175000
},
{
"Marque": "dodge",
"Critère Modèle": "contains",
"Modèle": "charger",
"Critère année": "",
"Année": null,
"Critère Kilométrage": "lessThan",
"Kilométrage": 175000
},
{
"Marque": "donkervoort",
"Critère Modèle": "matchAll",
"Modèle": "",
"Critère année": "",
"Année": null,
"Critère Kilométrage": "lessThan",
"Kilométrage": 175000
},
{
"Marque": "ferrari",
"Critère Modèle": "matchAll",
"Modèle": "",
"Critère année": "",
"Année": null,
"Critère Kilométrage": "lessThan",
"Kilométrage": 175000
},
{
"Marque": "fiat",
"Critère Modèle": "contains",
"Modèle": "abarth",
"Critère année": "",
"Année": null,
"Critère Kilométrage": "lessThan",
"Kilométrage": 175000
},
{
"Marque": "fiat",
"Critère Modèle": "contains",
"Modèle": "dino",
"Critère année": "",
"Année": null,
"Critère Kilométrage": "lessThan",
"Kilométrage": 175000
},
{
"Marque": "ford",
"Critère Modèle": "endsWith",
"Modèle": "RS",
"Critère année": "",
"Année": null,
"Critère Kilométrage": "lessThan",
"Kilométrage": 175000
},
{
"Marque": "ford",
"Critère Modèle": "contains",
"Modèle": "rs",
"Critère année": "",
"Année": null,
"Critère Kilométrage": "lessThan",
"Kilométrage": 175000
},
{
"Marque": "ford",
"Critère Modèle": "contains",
"Modèle": "mustang",
"Critère année": "",
"Année": null,
"Critère Kilométrage": "lessThan",
"Kilométrage": 175000
},
{
"Marque": "ford",
"Critère Modèle": "contains",
"Modèle": "gt",
"Critère année": "",
"Année": null,
"Critère Kilométrage": "lessThan",
"Kilométrage": 175000
},
{
"Marque": "ford",
"Critère Modèle": "contains",
"Modèle": "escort",
"Critère année": "",
"Année": null,
"Critère Kilométrage": "lessThan",
"Kilométrage": 175000
},
{
"Marque": "ford",
"Critère Modèle": "contains",
"Modèle": "mexico",
"Critère année": "",
"Année": null,
"Critère Kilométrage": "lessThan",
"Kilométrage": 175000
},
{
"Marque": "honda",
"Critère Modèle": "contains",
"Modèle": "type r",
"Critère année": "",
"Année": null,
"Critère Kilométrage": "lessThan",
"Kilométrage": 175000
},
{
"Marque": "honda",
"Critère Modèle": "contains",
"Modèle": "r type",
"Critère année": "",
"Année": null,
"Critère Kilométrage": "lessThan",
"Kilométrage": 175000
},
{
"Marque": "honda",
"Critère Modèle": "contains",
"Modèle": "s2000",
"Critère année": "",
"Année": null,
"Critère Kilométrage": "lessThan",
"Kilométrage": 175000
},
{
"Marque": "honda",
"Critère Modèle": "contains",
"Modèle": "s 2000",
"Critère année": "",
"Année": null,
"Critère Kilométrage": "lessThan",
"Kilométrage": 175000
},
{
"Marque": "honda",
"Critère Modèle": "contains",
"Modèle": "nsx",
"Critère année": "",
"Année": null,
"Critère Kilométrage": "lessThan",
"Kilométrage": 175000
},
{
"Marque": "jaguar",
"Critère Modèle": "contains",
"Modèle": "xk",
"Critère année": "",
"Année": null,
"Critère Kilométrage": "lessThan",
"Kilométrage": 175000
},
{
"Marque": "jaguar",
"Critère Modèle": "contains",
"Modèle": "ftype",
"Critère année": "",
"Année": null,
"Critère Kilométrage": "lessThan",
"Kilométrage": 175000
},
{
"Marque": "jaguar",
"Critère Modèle": "contains",
"Modèle": "f type",
"Critère année": "",
"Année": null,
"Critère Kilométrage": "lessThan",
"Kilométrage": 175000
},
{
"Marque": "jaguar",
"Critère Modèle": "contains",
"Modèle": "xj",
"Critère année": "",
"Année": null,
"Critère Kilométrage": "lessThan",
"Kilométrage": 175000
},
{
"Marque": "jaguar",
"Critère Modèle": "matchAll",
"Modèle": "",
"Critère année": "lessThan",
"Année": 1990,
"Critère Kilométrage": "lessThan",
"Kilométrage": 175000
},
{
"Marque": "koenigsegg",
"Critère Modèle": "matchAll",
"Modèle": "",
"Critère année": "",
"Année": null,
"Critère Kilométrage": "lessThan",
"Kilométrage": 175000
},
{
"Marque": "lamborghini",
"Critère Modèle": "matchAll",
"Modèle": "",
"Critère année": "",
"Année": null,
"Critère Kilométrage": "lessThan",
"Kilométrage": 175000
},
{
"Marque": "lancia",
"Critère Modèle": "contains",
"Modèle": "delta",
"Critère année": "",
"Année": null,
"Critère Kilométrage": "lessThan",
"Kilométrage": 175000
},
{
"Marque": "lancia",
"Critère Modèle": "contains",
"Modèle": "stratos",
"Critère année": "",
"Année": null,
"Critère Kilométrage": "lessThan",
"Kilométrage": 175000
},
{
"Marque": "lancia",
"Critère Modèle": "contains",
"Modèle": "aurelia",
"Critère année": "",
"Année": null,
"Critère Kilométrage": "lessThan",
"Kilométrage": 175000
},
{
"Marque": "lancia",
"Critère Modèle": "contains",
"Modèle": "37",
"Critère année": "",
"Année": null,
"Critère Kilométrage": "lessThan",
"Kilométrage": 175000
},
{
"Marque": "lancia",
"Critère Modèle": "contains",
"Modèle": "hyena",
"Critère année": "",
"Année": null,
"Critère Kilométrage": "lessThan",
"Kilométrage": 175000
},
{
"Marque": "lancia",
"Critère Modèle": "contains",
"Modèle": "flaminia",
"Critère année": "",
"Année": null,
"Critère Kilométrage": "lessThan",
"Kilométrage": 175000
},
{
"Marque": "lancia",
"Critère Modèle": "contains",
"Modèle": "flavia",
"Critère année": "",
"Année": null,
"Critère Kilométrage": "lessThan",
"Kilométrage": 175000
},
{
"Marque": "lancia",
"Critère Modèle": "contains",
"Modèle": "gamma",
"Critère année": "",
"Année": null,
"Critère Kilométrage": "lessThan",
"Kilométrage": 175000
},
{
"Marque": "lancia",
"Critère Modèle": "contains",
"Modèle": "appia",
"Critère année": "",
"Année": null,
"Critère Kilométrage": "lessThan",
"Kilométrage": 175000
},
{
"Marque": "lancia",
"Critère Modèle": "contains",
"Modèle": "beta montecarlo",
"Critère année": "",
"Année": null,
"Critère Kilométrage": "lessThan",
"Kilométrage": 175000
},
{
"Marque": "lancia",
"Critère Modèle": "contains",
"Modèle": "beta mc",
"Critère année": "",
"Année": null,
"Critère Kilométrage": "lessThan",
"Kilométrage": 175000
},
{
"Marque": "lancia",
"Critère Modèle": "contains",
"Modèle": "fulvia",
"Critère année": "",
"Année": null,
"Critère Kilométrage": "lessThan",
"Kilométrage": 175000
},
{
"Marque": "lancia",
"Critère Modèle": "contains",
"Modèle": "integrale",
"Critère année": "",
"Année": null,
"Critère Kilométrage": "lessThan",
"Kilométrage": 175000
},
{
"Marque": "land rover",
"Critère Modèle": "matchAll",
"Modèle": "",
"Critère année": "",
"Année": null,
"Critère Kilométrage": "lessThan",
"Kilométrage": 175000
},
{
"Marque": "lexus",
"Critère Modèle": "contains",
"Modèle": "lfa",
"Critère année": "",
"Année": null,
"Critère Kilométrage": "lessThan",
"Kilométrage": 175000
},
{
"Marque": "lotus",
"Critère Modèle": "matchAll",
"Modèle": "",
"Critère année": "",
"Année": null,
"Critère Kilométrage": "lessThan",
"Kilométrage": 175000
},
{
"Marque": "maserati",
"Critère Modèle": "matchAll",
"Modèle": "",
"Critère année": "",
"Année": null,
"Critère Kilométrage": "lessThan",
"Kilométrage": 140000
},
{
"Marque": "mazda",
"Critère Modèle": "contains",
"Modèle": "rx",
"Critère année": "",
"Année": null,
"Critère Kilométrage": "lessThan",
"Kilométrage": 175000
},
{
"Marque": "mclaren",
"Critère Modèle": "matchAll",
"Modèle": "",
"Critère année": "",
"Année": null,
"Critère Kilométrage": "lessThan",
"Kilométrage": 120000
},
{
"Marque": "mercedes",
"Critère Modèle": "contains",
"Modèle": "amg",
"Critère année": "",
"Année": null,
"Critère Kilométrage": "lessThan",
"Kilométrage": 175000
},
{
"Marque": "mercedes",
"Critère Modèle": "contains",
"Modèle": "gt",
"Critère année": "",
"Année": null,
"Critère Kilométrage": "lessThan",
"Kilométrage": 80000
},
{
"Marque": "mercedes",
"Critère Modèle": "contains",
"Modèle": "sl",
"Critère année": "",
"Année": null,
"Critère Kilométrage": "lessThan",
"Kilométrage": 175000
},
{
"Marque": "mercedes",
"Critère Modèle": "contains",
"Modèle": "500",
"Critère année": "",
"Année": null,
"Critère Kilométrage": "lessThan",
"Kilométrage": 175000
},
{
"Marque": "mercedes",
"Critère Modèle": "contains",
"Modèle": "420",
"Critère année": "",
"Année": null,
"Critère Kilométrage": "lessThan",
"Kilométrage": 175000
},
{
"Marque": "mercedes",
"Critère Modèle": "contains",
"Modèle": "class g",
"Critère année": "",
"Année": null,
"Critère Kilométrage": "lessThan",
"Kilométrage": 175000
},
{
"Marque": "mercedes",
"Critère Modèle": "contains",
"Modèle": "classe g",
"Critère année": "",
"Année": null,
"Critère Kilométrage": "lessThan",
"Kilométrage": 175000
},
{
"Marque": "mg",
"Critère Modèle": "matchAll",
"Modèle": "",
"Critère année": "",
"Année": null,
"Critère Kilométrage": "lessThan",
"Kilométrage": 175000
},
{
"Marque": "mini",
"Critère Modèle": "contains",
"Modèle": "gp",
"Critère année": "",
"Année": null,
"Critère Kilométrage": "lessThan",
"Kilométrage": 175000
},
{
"Marque": "mini",
"Critère Modèle": "contains",
"Modèle": "john cooper works",
"Critère année": "",
"Année": null,
"Critère Kilométrage": "lessThan",
"Kilométrage": 175000
},
{
"Marque": "mini",
"Critère Modèle": "matchAll",
"Modèle": "",
"Critère année": "lessThan",
"Année": 1990,
"Critère Kilométrage": "lessThan",
"Kilométrage": 175000
},
{
"Marque": "mitsubishi",
"Critère Modèle": "contains",
"Modèle": "lancer",
"Critère année": "",
"Année": null,
"Critère Kilométrage": "lessThan",
"Kilométrage": 175000
},
{
"Marque": "morgan",
"Critère Modèle": "matchAll",
"Modèle": "",
"Critère année": "",
"Année": null,
"Critère Kilométrage": "lessThan",
"Kilométrage": 175000
},
{
"Marque": "nissan",
"Critère Modèle": "contains",
"Modèle": "skyline",
"Critère année": "",
"Année": null,
"Critère Kilométrage": "lessThan",
"Kilométrage": 175000
},
{
"Marque": "nissan",
"Critère Modèle": "contains",
"Modèle": "silvia",
"Critère année": "",
"Année": null,
"Critère Kilométrage": "lessThan",
"Kilométrage": 175000
},
{
"Marque": "nissan",
"Critère Modèle": "contains",
"Modèle": "240",
"Critère année": "",
"Année": null,
"Critère Kilométrage": "lessThan",
"Kilométrage": 175000
},
{
"Marque": "nissan",
"Critère Modèle": "contains",
"Modèle": "nismo",
"Critère année": "",
"Année": null,
"Critère Kilométrage": "lessThan",
"Kilométrage": 175000
},
{
"Marque": "nissan",
"Critère Modèle": "contains",
"Modèle": "r390",
"Critère année": "",
"Année": null,
"Critère Kilométrage": "lessThan",
"Kilométrage": 175000
},
{
"Marque": "nissan",
"Critère Modèle": "contains",
"Modèle": "r 390",
"Critère année": "",
"Année": null,
"Critère Kilométrage": "lessThan",
"Kilométrage": 175000
},
{
"Marque": "nissan",
"Critère Modèle": "contains",
"Modèle": "gtr",
"Critère année": "",
"Année": null,
"Critère Kilométrage": "lessThan",
"Kilométrage": 175000
},
{
"Marque": "nissan",
"Critère Modèle": "contains",
"Modèle": "gt r",
"Critère année": "",
"Année": null,
"Critère Kilométrage": "lessThan",
"Kilométrage": 175000
},
{
"Marque": "noble",
"Critère Modèle": "matchAll",
"Modèle": "",
"Critère année": "",
"Année": null,
"Critère Kilométrage": "lessThan",
"Kilométrage": 175000
},
{
"Marque": "pagani",
"Critère Modèle": "matchAll",
"Modèle": "",
"Critère année": "",
"Année": null,
"Critère Kilométrage": "lessThan",
"Kilométrage": 175000
},
{
"Marque": "peugeot",
"Critère Modèle": "contains",
"Modèle": "turbo",
"Critère année": "",
"Année": null,
"Critère Kilométrage": "lessThan",
"Kilométrage": 175000
},
{
"Marque": "peugeot",
"Critère Modèle": "contains",
"Modèle": "t16",
"Critère année": "",
"Année": null,
"Critère Kilométrage": "lessThan",
"Kilométrage": 175000
},
{
"Marque": "peugeot",
"Critère Modèle": "contains",
"Modèle": "t 16",
"Critère année": "",
"Année": null,
"Critère Kilométrage": "lessThan",
"Kilométrage": 175000
},
{
"Marque": "porsche",
"Critère Modèle": "contains",
"Modèle": "356",
"Critère année": "",
"Année": null,
"Critère Kilométrage": "lessThan",
"Kilométrage": 175000
},
{
"Marque": "porsche",
"Critère Modèle": "contains",
"Modèle": "550",
"Critère année": "",
"Année": null,
"Critère Kilométrage": "lessThan",
"Kilométrage": 175000
},
{
"Marque": "porsche",
"Critère Modèle": "contains",
"Modèle": "718",
"Critère année": "",
"Année": null,
"Critère Kilométrage": "lessThan",
"Kilométrage": 100000
},
{
"Marque": "porsche",
"Critère Modèle": "contains",
"Modèle": "spyder",
"Critère année": "",
"Année": null,
"Critère Kilométrage": "lessThan",
"Kilométrage": 100000
},
{
"Marque": "porsche",
"Critère Modèle": "contains",
"Modèle": "speedster",
"Critère année": "",
"Année": null,
"Critère Kilométrage": "lessThan",
"Kilométrage": 100000
},
{
"Marque": "porsche",
"Critère Modèle": "contains",
"Modèle": "901",
"Critère année": "",
"Année": null,
"Critère Kilométrage": "lessThan",
"Kilométrage": 175000
},
{
"Marque": "porsche",
"Critère Modèle": "contains",
"Modèle": "911",
"Critère année": "",
"Année": null,
"Critère Kilométrage": "lessThan",
"Kilométrage": 175000
},
{
"Marque": "porsche",
"Critère Modèle": "contains",
"Modèle": "914",
"Critère année": "",
"Année": null,
"Critère Kilométrage": "lessThan",
"Kilométrage": 175000
},
{
"Marque": "porsche",
"Critère Modèle": "contains",
"Modèle": "918",
"Critère année": "",
"Année": null,
"Critère Kilométrage": "lessThan",
"Kilométrage": 50000
},
{
"Marque": "porsche",
"Critère Modèle": "contains",
"Modèle": "964",
"Critère année": "",
"Année": null,
"Critère Kilométrage": "lessThan",
"Kilométrage": 200000
},
{
"Marque": "porsche",
"Critère Modèle": "contains",
"Modèle": "993",
"Critère année": "",
"Année": null,
"Critère Kilométrage": "lessThan",
"Kilométrage": 175000
},
{
"Marque": "porsche",
"Critère Modèle": "contains",
"Modèle": "996",
"Critère année": "",
"Année": null,
"Critère Kilométrage": "lessThan",
"Kilométrage": 150000
},
{
"Marque": "porsche",
"Critère Modèle": "contains",
"Modèle": "997",
"Critère année": "",
"Année": null,
"Critère Kilométrage": "lessThan",
"Kilométrage": 100000
},
{
"Marque": "porsche",
"Critère Modèle": "contains",
"Modèle": "991",
"Critère année": "",
"Année": null,
"Critère Kilométrage": "lessThan",
"Kilométrage": 100000
},
{
"Marque": "porsche",
"Critère Modèle": "contains",
"Modèle": "992",
"Critère année": "",
"Année": null,
"Critère Kilométrage": "lessThan",
"Kilométrage": 100000
},
{
"Marque": "porsche",
"Critère Modèle": "contains",
"Modèle": "959",
"Critère année": "",
"Année": null,
"Critère Kilométrage": "lessThan",
"Kilométrage": 175000
},
{
"Marque": "porsche",
"Critère Modèle": "contains",
"Modèle": "935",
"Critère année": "",
"Année": null,
"Critère Kilométrage": "lessThan",
"Kilométrage": 175000
},
{
"Marque": "porsche",
"Critère Modèle": "contains",
"Modèle": "917",
"Critère année": "",
"Année": null,
"Critère Kilométrage": "lessThan",
"Kilométrage": 175000
},
{
"Marque": "porsche",
"Critère Modèle": "contains",
"Modèle": "930",
"Critère année": "",
"Année": null,
"Critère Kilométrage": "lessThan",
"Kilométrage": 170000
},
{
"Marque": "porsche",
"Critère Modèle": "contains",
"Modèle": "987",
"Critère année": "",
"Année": null,
"Critère Kilométrage": "lessThan",
"Kilométrage": 130000
},
{
"Marque": "porsche",
"Critère Modèle": "contains",
"Modèle": "944",
"Critère année": "",
"Année": null,
"Critère Kilométrage": "lessThan",
"Kilométrage": 175000
},
{
"Marque": "porsche",
"Critère Modèle": "contains",
"Modèle": "cayman",
"Critère année": "",
"Année": null,
"Critère Kilométrage": "lessThan",
"Kilométrage": 100000
},
{
"Marque": "porsche",
"Critère Modèle": "contains",
"Modèle": "boxster",
"Critère année": "",
"Année": null,
"Critère Kilométrage": "lessThan",
"Kilométrage": 100000
},
{
"Marque": "porsche",
"Critère Modèle": "contains",
"Modèle": "carrera gt",
"Critère année": "",
"Année": null,
"Critère Kilométrage": "lessThan",
"Kilométrage": 100000
},
{
"Marque": "range rover",
"Critère Modèle": "matchAll",
"Modèle": "",
"Critère année": "",
"Année": null,
"Critère Kilométrage": "lessThan",
"Kilométrage": 150000
},
{
"Marque": "renault",
"Critère Modèle": "isExactly",
"Modèle": "r5 turbo",
"Critère année": "",
"Année": null,
"Critère Kilométrage": "lessThan",
"Kilométrage": 175000
},
{
"Marque": "renault",
"Critère Modèle": "isExactly",
"Modèle": "r5 turbo 2",
"Critère année": "",
"Année": null,
"Critère Kilométrage": "lessThan",
"Kilométrage": 175000
},
{
"Marque": "renault",
"Critère Modèle": "isExactly",
"Modèle": "r5 turbo2",
"Critère année": "",
"Année": null,
"Critère Kilométrage": "lessThan",
"Kilométrage": 175000
},
{
"Marque": "renault",
"Critère Modèle": "isExactly",
"Modèle": "r5 alpine",
"Critère année": "",
"Année": null,
"Critère Kilométrage": "lessThan",
"Kilométrage": 175000
},
{
"Marque": "renault",
"Critère Modèle": "isExactly",
"Modèle": "clio williams",
"Critère année": "",
"Année": null,
"Critère Kilométrage": "lessThan",
"Kilométrage": 175000
},
{
"Marque": "renault",
"Critère Modèle": "contains",
"Modèle": "rs",
"Critère année": "",
"Année": null,
"Critère Kilométrage": "lessThan",
"Kilométrage": 175000
},
{
"Marque": "renault",
"Critère Modèle": "contains",
"Modèle": "alpine",
"Critère année": "",
"Année": null,
"Critère Kilométrage": "lessThan",
"Kilométrage": 175000
},
{
"Marque": "renault",
"Critère Modèle": "contains",
"Modèle": "spider",
"Critère année": "",
"Année": null,
"Critère Kilométrage": "lessThan",
"Kilométrage": 175000
},
{
"Marque": "rolls royce",
"Critère Modèle": "matchAll",
"Modèle": "",
"Critère année": "",
"Année": null,
"Critère Kilométrage": "lessThan",
"Kilométrage": 175000
},
{
"Marque": "shelby",
"Critère Modèle": "matchAll",
"Modèle": "",
"Critère année": "",
"Année": null,
"Critère Kilométrage": "lessThan",
"Kilométrage": 175000
},
{
"Marque": "subaru",
"Critère Modèle": "matchAll",
"Modèle": "",
"Critère année": "",
"Année": null,
"Critère Kilométrage": "lessThan",
"Kilométrage": 175000
},
{
"Marque": "toyota",
"Critère Modèle": "contains",
"Modèle": "supra",
"Critère année": "",
"Année": null,
"Critère Kilométrage": "lessThan",
"Kilométrage": 175000
},
{
"Marque": "toyota",
"Critère Modèle": "contains",
"Modèle": "gr",
"Critère année": "",
"Année": null,
"Critère Kilométrage": "lessThan",
"Kilométrage": 175000
},
{
"Marque": "triumph",
"Critère Modèle": "matchAll",
"Modèle": "",
"Critère année": "",
"Année": null,
"Critère Kilométrage": "lessThan",
"Kilométrage": 175000
},
{
"Marque": "tvr",
"Critère Modèle": "matchAll",
"Modèle": "",
"Critère année": "",
"Année": null,
"Critère Kilométrage": "lessThan",
"Kilométrage": 175000
},
{
"Marque": "volkswagen",
"Critère Modèle": "contains",
"Modèle": "r32",
"Critère année": "",
"Année": null,
"Critère Kilométrage": "lessThan",
"Kilométrage": 175000
},
{
"Marque": "volkswagen",
"Critère Modèle": "contains",
"Modèle": "r 32",
"Critère année": "",
"Année": null,
"Critère Kilométrage": "lessThan",
"Kilométrage": 175000
},
{
"Marque": "volkswagen",
"Critère Modèle": "contains",
"Modèle": "rsi",
"Critère année": "",
"Année": null,
"Critère Kilométrage": "lessThan",
"Kilométrage": 175000
},
{
"Marque": "volkswagen",
"Critère Modèle": "contains",
"Modèle": "gti",
"Critère année": "",
"Année": null,
"Critère Kilométrage": "lessThan",
"Kilométrage": 175000
},
{
"Marque": "volvo",
"Critère Modèle": "contains",
"Modèle": "850r",
"Critère année": "",
"Année": null,
"Critère Kilométrage": "lessThan",
"Kilométrage": 175000
},
{
"Marque": "volvo",
"Critère Modèle": "contains",
"Modèle": "850 r",
"Critère année": "lessThan",
"Année": null,
"Critère Kilométrage": "lessThan",
"Kilométrage": 175000
},
{
"Marque": "volvo",
"Critère Modèle": "contains",
"Modèle": "v70r",
"Critère année": "",
"Année": null,
"Critère Kilométrage": "lessThan",
"Kilométrage": 175000
},
{
"Marque": "volvo",
"Critère Modèle": "contains",
"Modèle": "v70 r",
"Critère année": "",
"Année": null,
"Critère Kilométrage": "lessThan",
"Kilométrage": 175000
},
{
"Marque": "volvo",
"Critère Modèle": "contains",
"Modèle": "t5r",
"Critère année": "",
"Année": null,
"Critère Kilométrage": "lessThan",
"Kilométrage": 175000
},
{
"Marque": "volvo",
"Critère Modèle": "contains",
"Modèle": "t5 r",
"Critère année": "",
"Année": null,
"Critère Kilométrage": "lessThan",
"Kilométrage": 175000
} ];

document.getElementById("wf-form-Sell-car").addEventListener("submit", function(event) {
checkInputs();
});



function checkInputs() {
  const inputKilo = document.getElementById("Entrer-le-kilom-trage-2").value;
  const inputMarque = document.getElementById("Entrer-la-marque-2").value.toLowerCase();
  const inputModele = document.getElementById("Mod-le-Version-2").value.toLowerCase();
  const inputAnnee = document.getElementById("Ann-e-2").value;

  vehicules.forEach(function (vehicule) {

    if (inputMarque === vehicule["Marque"].toLowerCase()) {
      console.log("+1 Entrer-la-marque-2");
      //Matches += 1;
      if (
      vehicule["Critère Modèle"] === "matchAll" ||
      (vehicule["Critère Modèle"] === "contains" && inputModele.includes(vehicule["Modèle"].toLowerCase())) ||
      (vehicule["Critère Modèle"] === "isExactly" && inputModele === vehicule["Modèle"].toLowerCase()) ||
      (vehicule["Critère Modèle"] === "startsWith" && inputModele.startsWith(vehicule["Modèle"].toLowerCase())) ||
      (vehicule["Critère Modèle"] === "endsWith" && inputModele.endsWith(vehicule["Modèle"].toLowerCase()))
    ) {
      console.log("+1 Mod-le-Version-2");
     // Matches += 1;
      if (inputAnnee != "" && (vehicule["Critère année"]=="lessThan" || vehicule["Critère année"]=="") && (inputAnnee >= vehicule["Année"] || vehicule["Année"] == "Null")) {
     // Matches += 1;
      console.log("+1 Ann-e-2");
        if (vehicule["Critère Kilométrage"]=="lessThan" && inputKilo <= vehicule["Kilométrage"]) {
      console.log("+1 Entrer-le-kilom-trage-2");
      Matches += 1;
    } 
    } 
    } 

}

  }
  )
  //If founded only 1 car
  if (Matches === 1) {
      console.log(Matches);
      console.log("Congratulations! All criteria match for 1 car.");
      dataLayer.push({
        'event': 'gtm.click',
        'eventAction': 'Yes interesting',
        'eventID': 176
      });
    } 
  //if founded 0 car
  if (Matches < 1) {
        console.log("Not interesting");
        dataLayer.push({
          'event': 'gtm.click',
          'eventAction': 'Not interesting',
          'eventID': 176
        });
    }
  //If founded more than 1 car
  if (Matches > 1) {
        console.log("Congratulations! All criteria match for More than 1 car");
        dataLayer.push({
          'event': 'gtm.click',
          'eventAction': 'Yes interesting',
          'eventID': 176
        });
    }
};


/*
    } else if (inputMarque != vehicule["Marque"].toLowerCase()) {
      console.log("Not interesting Entrer-la-marque-2");
    }

    if (
      vehicule["Critère Modèle"] === "matchAll" ||
      (vehicule["Critère Modèle"] === "contains" && inputModele.includes(vehicule["Modèle"].toLowerCase())) ||
      (vehicule["Critère Modèle"] === "isExactly" && inputModele === vehicule["Modèle"].toLowerCase()) ||
      (vehicule["Critère Modèle"] === "startsWith" && inputModele.startsWith(vehicule["Modèle"].toLowerCase())) ||
      (vehicule["Critère Modèle"] === "endsWith" && inputModele.endsWith(vehicule["Modèle"].toLowerCase()))
    ) {
      console.log("+1 Mod-le-Version-2");
      Matches += 1;
    } else if (
      vehicule["Critère Modèle"] != "matchAll" ||
      (vehicule["Critère Modèle"] != "contains" && inputModele.includes(vehicule["Modèle"].toLowerCase())) ||
      (vehicule["Critère Modèle"] != "isExactly" && inputModele != vehicule["Modèle"].toLowerCase()) ||
      (vehicule["Critère Modèle"] != "startsWith" && inputModele.startsWith(vehicule["Modèle"].toLowerCase())) ||
      (vehicule["Critère Modèle"] != "endsWith" && inputModele.endsWith(vehicule["Modèle"].toLowerCase()))
    ) {
      console.log("Not interesting Mod-le-Version-2");
    }

    else if (inputAnnee < vehicule["Année"]) {
      console.log("Not interesting Ann-e-2");
    }
   else if (inputKilo > vehicule["Kilométrage"]) {
      console.log("Not interesting Entrer-le-kilom-trage-2");
    }
  });
*/


